...
/*   Updated: 2024/08/30 14:32:47 by rgallien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (current && current->type != END)
	{
		...
		while (current->str[tab[0]])
		{
			if (current->str[tab[0]] == 39)
				expand_simple(current->str, new, (int**)&tab);
			else if (current->str[tab[0]] == '"')
				expand_double(current->str, new, (int**)&tab, env);
			else if (current->str[tab[0]] == '$')
				found_variable(current->str, new, (int**)&tab, env);
			else
